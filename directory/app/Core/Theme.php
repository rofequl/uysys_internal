...
class Theme
{
    ...
    {
        return sprintf('<link rel="shortcut icon" href="%s" />', asset(config('settings.BSD_THEME_ASSETS.favicon')));
    }
    ...
    function includeFonts()
    {
        ...
        foreach (config('settings.BSD_THEME_ASSETS.fonts') as $url) {
    ...
    function getGlobalAssets($type = 'js')
    {
        ...
        }, config('settings.BSD_THEME_ASSETS.global.'.$type));
    }

    /**
     * Add multiple vendors to the page by name. Refer to settings BSD_THEME_VENDORS
     *
     * @param $vendors
     *
     * @return array
    ...
    /**
     * Add single vendor to the page by name. Refer to settings BSD_THEME_VENDORS
     *
     * @param $vendor
     *
     * @return void
    ...
    /**
     * Get vendor files from settings. Refer to settings BSD_THEME_VENDORS
     *
     * @param $type
     *
     * @return array
    ...
    function getVendors($type)
    {
        ...
        foreach (self::$vendorFiles as $vendor) {
            $vendors = config('settings.BSD_THEME_VENDORS.'.$vendor);
    ...
    {
        $type = config('settings.BSD_THEME_ICONS', 'duotone');
